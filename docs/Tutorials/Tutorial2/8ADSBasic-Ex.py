...
def main():
    ...
    final_list = ADS.ADSroutine(init_list, tol, Nmax, eFun)
    ...
    for j in range(final_domain.shape[2]):
        for k in range(4):
            ax2.plot3D(final_domain[0, Ns * k : Ns * (k + 1), j],final_domain[1, Ns * k : Ns * (k + 1), j], final_manifold[ Ns * k : Ns * (k + 1), j],color='black')
            ax2.plot3D(perimeter[ Ns * k : Ns * (k + 1),0]+x[0].cons(), perimeter[ Ns * k : Ns * (k + 1),1]+x[1].cons(), XF[ Ns * k : Ns * (k + 1)], color='green')
            ax2.plot3D(perimeter[ Ns * k : Ns * (k + 1),0]+x[0].cons(), perimeter[ Ns * k : Ns * (k + 1),1]+x[1].cons(), XF_single[ Ns * k : Ns * (k + 1)], color='red')
    ax2.set_xlabel('x', fontsize = 16)
    ax2.set_ylabel('y', fontsize = 16)
    ax2.set_zlabel('z', fontsize = 16)
    ax2.legend(("ADS", "Exact", "10th order"),
